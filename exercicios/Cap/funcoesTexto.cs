    ...
    class funcoesTexto
    {
        static void Main (string[] args)
        {
            ...
            WriteLine($"texto atual: {empresa}");
                var novaEmpresa = empresa.Replace("Microsoft", "Google").Replace("Corporation", "Corp");
            ...
            foreach(var p in cesta)
            {
                ...
                // mesmo que: qtde = qtde + Convert.ToInt32(p.Substring(0, p.IndexOf(" ")));
                {
                    WriteLine($"Qtde total: {qtde:N0}");
                }
                WriteLine("IsNullOrEmpty verifica se a string está nula ou vazia");
                //null seria como um estado da variavel, empty seria algo como "", apagado ou vazio.
                    string nome = "Renato";
